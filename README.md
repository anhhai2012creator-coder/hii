...
<head>
  ...
  <script>
    ...
    window.loadPeerJs = function loadPeerJs() {
      if (window.Peer) return Promise.resolve(window.Peer);
      if (window.__peerJsLoading) return window.__peerJsLoading;

      window.__peerJsLoading = new Promise(function(resolve, reject) {
        var index = 0;
        function tryNext() {
          if (window.Peer) return resolve(window.Peer);
          if (index >= window.PEERJS_CDNS.length) {
            window.__peerJsLoading = null;
            return reject(new Error('Không tải được thư viện PeerJS từ CDN.'));
          }
          var src = window.PEERJS_CDNS[index++];
...
<body>
  <div id="gameWrap">
    <div id="canvasBox">
      ...
      <div id="debugBox">Sẵn sàng. Bấm “Kết nối PeerJS”.</div>
      <div id="toast"></div>
      <div id="actionBar">
    ...
    <div class="panel">
      ...
      <div class="grid">
        <div class="card">
          ...
          <p class="small">Bước 1: bấm kết nối để lấy Peer ID. Nếu GitHub bị kẹt, nhìn dòng trạng thái góc trên bên trái để biết lỗi.</p>
        </div>
        <div class="card">
          <h2>Host / Join</h2>
...
(() => {
  ...
  const PROTOCOL = 'among-us-mini-peerjs-v2';
  let peer = null;
  ...
  function buildPlayerProfile() {
    const old = myId ? state.players[myId] : null;
    const p = old || randSpawn();
    return {
      id: myId,
      name: safeName(),
      color: myColor(),
      ...
      connected: true,
      tasksDone: old?.tasksDone || {},
      killReadyAt: old?.killReadyAt || 0,
      emergencyUsed: old?.emergencyUsed || false
    };
  }

  function createLocalPlayer() {
    if (!myId) return;
    state.players[myId] = buildPlayerProfile();
  }

  function peerErrorText(err) {
    const type = err?.type || err?.name || 'unknown';
    const msg = err?.message ? ' - ' + err.message : '';
    if (type === 'browser-incompatible') return 'Trình duyệt không hỗ trợ WebRTC.';
    if (type === 'network' || type === 'server-error' || type === 'socket-error' || type === 'socket-closed') return 'Không kết nối được máy chủ PeerJS. Kiểm tra mạng/CDN/HTTPS.';
    if (type === 'peer-unavailable') return 'Không tìm thấy phòng. Kiểm tra Peer ID của host.';
    return 'Lỗi PeerJS: ' + type + msg;
  }
  ...
  function hardResetConnection() {
    ...
    $('myId').textContent = '...';
    $('hostId').textContent = '...';
    $('copyId').textContent = 'chưa có';
    $('btnCreatePeer').disabled = false;
    $('btnHost').disabled = true;
    $('btnJoin').disabled = true;
    setStatus('Đã reset kết nối. Bấm Kết nối PeerJS lại.');
  }

  async function connectPeer() {
    if (peer && !peer.destroyed) {
      toast('PeerJS đang chạy rồi. Peer ID: ' + (myId || 'đang lấy...'));
      return;
    }
    $('btnCreatePeer').disabled = true;
    setStatus('Đang tải thư viện PeerJS...');

    try {
      await window.loadPeerJs();
    } catch (err) {
      $('btnCreatePeer').disabled = false;
      setStatus('Không tải được PeerJS');
      toast(err.message || 'Không tải được PeerJS. Kiểm tra mạng rồi thử lại.');
      return;
    }

    if (!window.Peer) {
      $('btnCreatePeer').disabled = false;
      setStatus('Thiếu PeerJS');
      toast('PeerJS chưa sẵn sàng. Hãy tải lại trang hoặc kiểm tra mạng.');
      return;
    }
    ...
    if (!window.isSecureContext) {
      $('btnCreatePeer').disabled = false;
      setStatus('Cần HTTPS');
      toast('Hãy mở bằng https://...github.io. Nếu mở file trực tiếp hoặc http thì WebRTC dễ không chạy.');
      return;
    }
  ...
  function startPeerAttempt() {
    cleanupPeer();
    const server = PEER_SERVER_OPTIONS[peerTryIndex] || PEER_SERVER_OPTIONS[0];
    setStatus('Đang kết nối: ' + server.label);
    $('btnCreatePeer').disabled = true;

    try {
      peer = new Peer(undefined, server.options);
    } catch (err) {
      $('btnCreatePeer').disabled = false;
      setStatus('Lỗi khởi tạo PeerJS');
      toast(peerErrorText(err));
      return;
    }
    ...
    peer.on('open', id => {
      ...
      setStatus('Online qua ' + server.label);
      $('btnCreatePeer').disabled = true;
      $('btnHost').disabled = false;
      $('btnJoin').disabled = false;
      createLocalPlayer();
      updateHUD();
      toast('Đã kết nối PeerJS. Copy Peer ID này cho bạn bè nếu bạn host.');
    });
    ...
    peer.on('disconnected', () => {
      setStatus('Mất tín hiệu, đang nối lại...');
      clearTimeout(reconnectTimer);
      reconnectTimer = setTimeout(() => {
        try { if (peer && !peer.destroyed) peer.reconnect(); } catch {}
      }, 900);
    });

    peer.on('close', () => {
      if (peerOpened) setStatus('Đã đóng kết nối');
    });

    peer.on('error', err => {
      console.error(err);
      const canTryNext = !peerOpened && peerTryIndex < PEER_SERVER_OPTIONS.length - 1;
      if (canTryNext) {
        peerTryIndex++;
        toast('Máy chủ PeerJS lỗi, đang thử cổng khác...');
        startPeerAttempt();
        return;
      }
      setStatus('Lỗi: ' + (err?.type || 'unknown'));
      toast(peerErrorText(err));
      if (!peerOpened) {
        cleanupPeer();
        $('btnCreatePeer').disabled = false;
  ...
  function setupHostConnection(conn) {
    ...
    conn.on('open', () => {
      conn.send({ type:'hello-host', protocol: PROTOCOL, hostId: myId, yourId: conn.peer });
    });
  ...
  function joinHost() {
    ...
    if (!peer || !peerOpened || !id || id === myId) return toast('Peer ID không hợp lệ hoặc bạn chưa kết nối PeerJS.');
    if (hostConn && hostConn.open) hostConn.close();
    host = false;
    hostPeerId = id;
    ...
    hostConn = peer.connect(id, {
      reliable: true,
      metadata: { protocol: PROTOCOL, name: safeName(), color: myColor() }
    });
    hostConn.on('open', () => {
      hostConn.send({ type:'join', protocol: PROTOCOL, player: buildPlayerProfile() });
    ...
    hostConn.on('close', () => {
      setStatus('Mất kết nối host');
      toast('Mất kết nối với host. Hãy vào lại bằng Peer ID của host.');
    });
  ...
  function becomeHost() {
    if (!peer || !peerOpened || !myId) return toast('Hãy bấm Kết nối PeerJS trước.');
    host = true;
    hostPeerId = myId;
  ...
  function handleHostMessage(from, msg) {
    ...
    if (msg.type === 'join') {
      if (msg.protocol && msg.protocol !== PROTOCOL) {
        ...
        if (conn?.open) conn.send({ type:'toast', msg:'Sai phiên bản game. Hãy tải lại trang.' });
        return;
      }
      const p = msg.player || {};
      ...
      state.players[from] = {
        ...p,
        id: from,
        name: String(p.name || 'Player').slice(0,14),
      ...
      const conn = conns.get(from);
      if (conn?.open) conn.send({ type:'welcome', protocol: PROTOCOL, hostId: myId, state, yourId: from });
      broadcastState();
    }
    if (msg.type === 'input') {
  ...
  function handleClientMessage(msg) {
    ...
    if (msg.type === 'hello-host') {
      if (msg.protocol && msg.protocol !== PROTOCOL) return toast('Host đang dùng phiên bản game khác. Hãy tải lại trang.');
      return;
    }
    if (msg.type === 'welcome') {
      if (msg.protocol && msg.protocol !== PROTOCOL) return toast('Host đang dùng phiên bản game khác. Hãy tải lại trang.');
      ...
      if (!state.players[myId]) createLocalPlayer();
      $('hostId').textContent = hostPeerId;
      setStatus('Đã đồng bộ phòng');
      updateHUD();
    }
    if (msg.type === 'state') {
      Object.assign(state, msg.state);
      if (!state.players[myId] && state.phase === 'menu') createLocalPlayer();
      updateMeetingUI();
      updateHUD();
  ...
  bindTap('btnCreatePeer', connectPeer);
  bindTap('btnHost', becomeHost);
  bindTap('btnJoin', joinHost);
  bindTap('btnStart', startGame);
  bindTap('btnUse', useAction);
  bindTap('btnKill', killAction);
  bindTap('btnReport', reportAction);
  bindTap('btnEmergency', emergencyAction);
  bindTap('btnSkip', () => sendToHost({ type:'vote', targetId:'skip' }));
  $('copyId').onclick = async () => {
    try {
      await navigator.clipboard.writeText(myId || '');
      toast('Đã copy Peer ID.');
    } catch {
      toast('Không copy tự động được. Hãy bôi đen Peer ID rồi copy thủ công.');
    }
  };
...
(() => {
  ...
  function connectPeer() {
    ...
    if (!window.isSecureContext) {
      toast('GitHub Pages phải mở bằng https:// thì PeerJS/WebRTC mới ổn định.');
      $('netStatus').textContent = 'cần https';
      return;
    }
    if (!window.Peer) {
      toast('Chưa tải được thư viện PeerJS. Hãy kiểm tra internet hoặc CDN.');
      $('netStatus').textContent = 'thiếu PeerJS';
      return;
    }
    peerTryIndex = 0;
    startPeerAttempt();
  ...
  function startPeerAttempt() {
    cleanupPeer();
    const server = PEER_SERVER_OPTIONS[peerTryIndex] || PEER_SERVER_OPTIONS[0];
    $('netStatus').textContent = 'đang kết nối: ' + server.label;
    $('btnCreatePeer').disabled = true;
    ...
    try {
      ...
      $('btnCreatePeer').disabled = false;
      toast(peerErrorText(err));
      $('netStatus').textContent = 'lỗi khởi tạo';
      return;
    }
    ...
    peer.on('open', id => {
      ...
      $('myId').textContent = id;
      $('copyId').textContent = id;
      $('netStatus').textContent = 'online qua ' + server.label;
      $('btnCreatePeer').disabled = true;
      $('btnHost').disabled = false;
      $('btnJoin').disabled = false;
      createLocalPlayer();
    ...
    peer.on('disconnected', () => {
      $('netStatus').textContent = 'mất tín hiệu, đang nối lại...';
      clearTimeout(reconnectTimer);
      reconnectTimer = setTimeout(() => {
        try { if (peer && !peer.destroyed) peer.reconnect(); } catch {}
      }, 900);
    ...
    peer.on('close', () => {
      if (peerOpened) $('netStatus').textContent = 'đã đóng kết nối';
    });
    ...
    peer.on('error', err => {
      ...
      $('netStatus').textContent = 'lỗi: ' + (err?.type || 'unknown');
      toast(peerErrorText(err));
      if (!peerOpened) {
        cleanupPeer();
        $('btnCreatePeer').disabled = false;
  ...
  function joinHost() {
    ...
    hostConn.on('open', () => {
      hostConn.send({ type:'join', protocol: PROTOCOL, player: buildPlayerProfile() });
      $('menu').classList.add('hidden');
      $('netStatus').textContent = 'đã vào phòng';
      toast('Đã vào phòng. Chờ host start game.');
    });
    hostConn.on('data', handleClientMessage);
    hostConn.on('close', () => {
      $('netStatus').textContent = 'mất kết nối host';
      toast('Mất kết nối với host. Hãy vào lại bằng Peer ID của host.');
    });
    hostConn.on('error', err => {
      $('netStatus').textContent = 'không vào được phòng';
      toast(peerErrorText(err));
    });
  }
  ...
  function handleClientMessage(msg) {
    ...
    if (msg.type === 'welcome') {
      ...
      if (!state.players[myId]) createLocalPlayer();
      $('hostId').textContent = hostPeerId;
      $('netStatus').textContent = 'đã đồng bộ phòng';
      updateHUD();
    }
    if (msg.type === 'state') {
      Object.assign(state, msg.state);
